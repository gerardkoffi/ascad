                            ...
                            <div class="single-practise-box margin-bottom-40">
                                <div class="content text-center" style="padding: 21px 30px 17px 30px;">
                                    ...
                                    <form action="{{route('user.card.store')}}" method="post">
                                        ...
                                        <hr>
                                        <strong class="title">Prix: {{$sub->price}} {{$gnl->cur}} / card</strong>
                                        <hr>
                                        <strong class="title">Votre quantité: {{$qty}}</strong>
                                        <hr>
                                        <strong class="title">Coût total: {{$total = $qty*$sub->price}} {{$gnl->cur}}</strong>
                                        <hr>
                                        <strong class="title">Solde actuel: {{$balance}} {{$gnl->cur}}</strong>
                                        <hr>
                                        <p></p>
                                        @if($balance >=  $total)
                                            <input type="submit" class="submit-btn" value="Purchase">
                                        @else
                                            <a href="{{url('home/deposit')}}" class="boxed-btn" >Déposez maintenant</a>
                                        @endif
                                    </form>
